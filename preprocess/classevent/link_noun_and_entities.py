...
def replace_with_nouns(db, nouns):
    for t in db.tweets:
        if t.tid in nouns:
            elements = nouns[t.tid]
            t.message.words = elements
        cleaned = set([w.lower() for w in t.message.words])
        t.message.words = list(cleaned)


def run(noun_db_file, entity_db_file, augmented_file, replaced_file):
    entity_db = load_clean_tweets(entity_db_file)
    noun_dict = load_tweet_nouns(noun_db_file)
    # clean_entities(entity_db)
    # entity_db.write_clean_tweets_to_file(entity_db_file)
    augment_with_nouns(entity_db, noun_dict)
    entity_db.write_clean_tweets_to_file(augmented_file)
    # replace_with_nouns(entity_db, noun_dict)
    # entity_db.write_clean_tweets_to_file(replaced_file)
...
if __name__ == '__main__':
    ...
    replaced_file = '/Users/chao/Dropbox/data/class_event/sample/tweets_noun.txt'
    if len(sys.argv) > 1:
        ...
        replaced_file = para['clean_tweet_noun']
    run(noun_db_file, entity_db_file, augmented_file, replaced_file)
